...
for path in args.list_path :
  ...
  for url in d['url'].keys() :
    ...
    for feat,cpt in features.iteritems() :
      ...
      if feat not in author_features :
        author_features[feat] = 0
      author_features[feat] += cpt
  authors_features[path] = author_features

nb_feat_author = {}
for path, d in authors_features.iteritems() :
  nb_feat_author[path] = len(d.keys())
#nb_feat_author = {(path, len(d.keys())) for path, d in authors_features.iteritems()}

sorted_author_features = sorted(nb_feat_author, key=nb_feat_author.get)

for a in sorted_author_features :
  print a, nb_feat_author[a]
